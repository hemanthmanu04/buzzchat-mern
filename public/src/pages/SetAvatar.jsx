...
const SetAvatar = () => {
  ...
  const fetchAvatars = async () => {
    const data = [];
    let retryCount = 0;
    const maxRetries = 3; // Maximum retries in case of rate limit errors
    const delayBetweenRetries = 5000; // Delay in milliseconds between retries
    while (retryCount < maxRetries) {
      try {
        ...
        if (error.response && error.response.status === 429) {
          retryCount++;
          console.log(`Rate limit exceeded. Retrying attempt ${retryCount}...`);
  ...
  const setProfilePicture = async () => {
    if (selectedAvatar === undefined) {
      ...
      if (data.isSet) {
        ...
        console.log("Avatar successfully set. Navigating...");
        navigate("/");
